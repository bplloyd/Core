rollingForecasts2 = function (y, forecastfunction, h = 12, level = c(80, 95), fcast_args=NULL, doPar=T)
{
  ...
  if(doPar==T) {
    ...
    parallel::clusterExport(cl, varlist = c("y", "n", "h", "level", "fcast_args", "forecastfunction"), envir = environment())
  ...
  #      )
    result = parallel::parLapply(cl,
                                X = seq_len(n),
                                fun = function(i) try(do.call(forecastfunction, append(list(y = subset(y,end = i),h = h,level = level), fcast_args)),
                                                      silent = TRUE))
